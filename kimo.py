...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@R_Y_I'))
	except BaseException:
		pass		
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@DevKiMo'))
	except BaseException :
		pass
@abbas.on(events.NewMessage)
async def join_channel(event):
...
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@l34444'))
	except BaseException :
		pass
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await kimo1.send_message(event.chat_id, f"تم الانتهاء من التجميع ")
                break
...
#        await event.reply("جاري تجميع النقاط")
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع ")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات حالياً 🤍') != -1:
                #await abbas.send_message(event.chat_id, "تم الانتهاء من التجميع")
                break
            msg_text = msgs.message  # الكود تمت كتابتهُ من قبل سورس كيمو ممنوع الخمط
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               #
               channel_entity = await abbas.get_entity(pot)
#               await abbas.send_message(pot, '**جاري بدأ عملية التجميع بواسطة كيمو**')
               await abbas.send_message(pot, '/start')
               await asyncio.sleep(2)
               msg0 = await abbas.get_messages(pot, limit=1)
               await msg0[0].click(2)
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinq = await abbas(JoinChannelRequest('Zy9999'))
        joinw = await abbas(JoinChannelRequest('Fvvvv'))
        joine = await abbas(JoinChannelRequest('DX5555'))
        joinr = await abbas(JoinChannelRequest('d8_8q'))
        joint = await abbas(JoinChannelRequest('billionbot3'))
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('R_Y_I'))
...
async def Abbas(event):
    if event.message.message.startswith("تمويل") and event.sender_id in Devkimo:
